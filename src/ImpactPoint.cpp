	...
	float time, const PRM_Template*)
{
	...
	node = parent->createNode("CusCentroidalVoronoi", "CVD1");
	
	// set parameters
	node->setFloat("forceMag", 0, time, forceMag);

	node->setFloat("forceDir", 0, time, forceDir[0]);
	node->setFloat("forceDir", 1, time, forceDir[1]);
	node->setFloat("forceDir", 2, time, forceDir[2]);

	node->setFloat("forceLoc", 0, time, forceLoc[0]);
	node->setFloat("forceLoc", 1, time, forceLoc[1]);
	node->setFloat("forceLoc", 2, time, forceLoc[2]);

	// connect the node
	input = parent->findNode("tetconform1");  // find /obj/geo1/tetConform1 as relative path
	if (input)
	{
		node->setInput(0, input);       // set first input to /obj/tetConform
	}
